      ...
      <p>Please {state=== 'Sign Up' ? "sign up" :"login"} to book appointment</p>
{
 ...
 <div className='w-full'>
      ...
      <button className='bg-rose-500 text-white w-full py-2 rounded-md text-base'>{state ==='Sign Up'? "Create Account" :"Login"}</button>
      {
        state ==="Sign Up" ?
        <p>Already have an account ?<span onClick={()=>setState('Login')} className='text-rose-500 underline cursor-pointer'> Login here</span></p>
        : <p>Create a new account? <span onClick={()=>setState('Sign up')} className='text-rose-500 underline cursor-pointer'>click here</span></p>
      }
    </div>
   </form>
  )
